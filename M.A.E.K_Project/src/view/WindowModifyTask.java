...
/**
 * The WindowModifyTask class represents a dialog window for modifying an existing task.
...
public class WindowModifyTask extends JDialog implements ActionListener {
	...
	/**
	 * Constructs a new WindowModifyTask dialog.
	 *
	...
	/**
	 * Handles action events for the modify and exit buttons.
	 *
			...
			if (comboBox.getSelectedIndex() != -1) {
				try {
					...
					if (!cont.modifyTask(new Task(task.getId(), name, description, dueDate, state, task.getUsername(), category))) {
						...
						JOptionPane.showMessageDialog(this, "An error has occurred modifying the task", "Error", JOptionPane.ERROR_MESSAGE);
					}
